...
local function LoadInventory(source, citizenid)
    ...
    if table.type(inventory) == "empty" then
        return loadedInventory
    end
...
local function AddItem(source, item, amount, slot, info)
    ...
    if (totalWeight + (itemInfo['weight'] * amount)) <= Config.MaxInventoryWeight then
        if (slot and Player.PlayerData.items[slot]) and (Player.PlayerData.items[slot].name:lower() == item:lower()) and
            ...
            TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'AddItem', 'green',
                '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source ..
                    ')** got item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name ..
                    ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[slot].amount)
        ...
        elseif itemInfo['unique'] or (not slot or slot == nil) or itemInfo['type'] == 'weapon' then
            for i = 1, Config.MaxInventorySlots, 1 do
                if Player.PlayerData.items[i] == nil then
                    ...
                    TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'AddItem', 'green',
                        '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' ..
                            source .. ')** got item: [slot:' .. i .. '], itemname: ' .. Player.PlayerData.items[i].name ..
                            ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[i].amount)
...
local function RemoveItem(source, item, amount, slot)
    ...
    if slot then
        if Player.PlayerData.items[slot].amount > amount then
            ...
            if not Player.Offline then
                TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red',
                    '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' ..
                        source .. ')** lost item: [slot:' .. slot .. '], itemname: ' ..
                        Player.PlayerData.items[slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' ..
                        Player.PlayerData.items[slot].amount)
            end
        ...
        elseif Player.PlayerData.items[slot].amount == amount then
            ...
            TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red',
                '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source ..
                    ')** lost item: [slot:' .. slot .. '], itemname: ' .. item .. ', removed amount: ' .. amount ..
                    ', item removed')
        ...
        for _, _slot in pairs(slots) do
            if Player.PlayerData.items[_slot].amount > amountToRemove then
                ...
                if not Player.Offline then
                    TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red',
                        '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' ..
                            source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' ..
                            Player.PlayerData.items[_slot].name .. ', removed amount: ' .. amount ..
                            ', new total amount: ' .. Player.PlayerData.items[_slot].amount)
                end
            ...
            elseif Player.PlayerData.items[_slot].amount == amountToRemove then
                ...
                TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red',
                    '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' ..
                        source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. item .. ', removed amount: ' ..
                        amount .. ', item removed')
...
local function HasItem(source, items, amount)
    ...
    if isTable then
        for k, v in pairs(items) do
            ...
            if item and
                ((amount and item.amount >= amount) or (not isArray and item.amount >= v) or (not amount and isArray)) then
                count = count + 1
            end
        end
...
local function SaveStashItems(stashId, items)
    if Stashes[stashId].label == "Stash-None" or not items then
        return
    end
...
local function SaveOwnedVehicleItems(plate, items)
    if Trunks[plate].label == "Trunk-None" or not items then
        return
    end
...
local function SaveOwnedGloveboxItems(plate, items)
    if Gloveboxes[plate].label == "Glovebox-None" or not items then
        return
    end
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
    ...
    if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
        ...
        TriggerEvent("qb-log:server:CreateLog", "drop", "New Item Drop", "red",
            "**" .. GetPlayerName(source) .. "** (citizenid: *" .. Player.PlayerData.citizenid .. "* | id: *" .. source ..
                "*) dropped new item; name: **" .. itemData.name .. "**, amount: **" .. itemAmount .. "**")
    ...
    if name and id then
        ...
        if name == "stash" then
            if Stashes[id] then
                if Stashes[id].isOpen then
                    ...
                    if Target then
                        TriggerClientEvent('inventory:client:CheckOpenState', Stashes[id].isOpen, name, id,
                            Stashes[id].label)
                    else
        ...
        elseif name == "trunk" then
            if Trunks[id] then
                if Trunks[id].isOpen then
                    ...
                    if Target then
                        TriggerClientEvent('inventory:client:CheckOpenState', Trunks[id].isOpen, name, id,
                            Trunks[id].label)
                    else
            ...
            if (Trunks[id] and Trunks[id].isOpen) or (QBCore.Shared.SplitStr(id, "PLZI")[2] and
                (Player.PlayerData.job.name ~= "police" or Player.PlayerData.job.type ~= "leo")) then
                secondInv.name = "none-inv"
        ...
        elseif name == "glovebox" then
            if Gloveboxes[id] then
                if Gloveboxes[id].isOpen then
                    ...
                    if Target then
                        TriggerClientEvent('inventory:client:CheckOpenState', Gloveboxes[id].isOpen, name, id,
                            Gloveboxes[id].label)
                    else
        ...
        elseif name == "otherplayer" then
            ...
            if OtherPlayer then
                ...
                if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.type == "leo") and
            ...
            if Drops[id] then
                if Drops[id].isOpen then
                    ...
                    if Target then
                        TriggerClientEvent('inventory:client:CheckOpenState', Drops[id].isOpen, name, id,
                            Drops[id].label)
                    else
...
RegisterNetEvent('QBCore:Server:UpdateObject', function()
    if source ~= '' then
        return
    end -- Safety check if the event was not called from the server.
...
RegisterNetEvent('inventory:server:combineItem', function(item, fromItem, toItem)
    ...
    if recipe and recipe.reward ~= item then
        return
    end
    if not recipeContains(recipe, fromItem) then
        return
    end
...
RegisterNetEvent('inventory:server:CraftAttachment', function(itemName, itemCosts, amount, toSlot, points)
    ...
    AddItem(src, itemName, amount, toSlot)
    Player.Functions.SetMetaData("attachmentcraftingrep",
        Player.PlayerData.metadata["attachmentcraftingrep"] + (points * amount))
    TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
end)
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
    ...
    if amount <= item.amount then
        ...
        if RemoveItem(src, item.name, amount, item.slot) then
            if item.name == 'cash' or item.name == 'blackmoney' then
...
QBCore.Functions.CreateCallback('QBCore:HasItem', function(source, cb, items, amount)
    print(
        "^3QBCore:HasItem is deprecated, please use QBCore.Functions.HasItem, it can be used on both server- and client-side and uses the same arguments.^0")
    local retval = false
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then
        return cb(false)
    ...
    if isTable then
        for k, v in pairs(items) do
            ...
            local item = GetItemByName(source, itemKV[kvIndex])
            if item and
                ((amount and item.amount >= amount) or (not amount and not isArray and item.amount >= v) or
...
QBCore.Commands.Add("resetinv", "Reset Inventory (Admin Only)", {{
    name = "type",
    help = "stash/trunk/glovebox"
}, {
    name = "id/plate",
    help = "ID of stash or license plate"
...
QBCore.Commands.Add("giveitem", "Give An Item (Admin Only)", {{
    ...
    name = "item",
    help = "Name of the item (not a label)"
}, {
    name = "amount",
    ...
    if Player then
        if itemData then
            ...
            elseif itemData["type"] == "weapon" then
                amount = 1
                info.serie = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) ..
                                          QBCore.Shared.RandomInt(1) .. QBCore.Shared.RandomStr(2) ..
                                          QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(4))
                info.quality = 100
            ...
            if AddItem(id, itemData["name"], amount, false, info) then
                QBCore.Functions.Notify(source, Lang:t("notify.yhg") .. GetPlayerName(id) .. " " .. amount .. " " ..
                    itemData["name"] .. "", "success")
            else
    ...
    for _, v in pairs(players) do
        ...
        if dist < 3.0 then
            ...
            TriggerClientEvent('chat:addMessage', v, {
                ...
                args = {"ID Card", item.info.citizenid, item.info.firstname, item.info.lastname, item.info.birthdate,
                        gender, item.info.nationality}
            })
        end
    end
end)
